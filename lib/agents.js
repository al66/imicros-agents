...
// login { serviceId, authToken } => { sessionToken, accessToken }
// verify { sessionToken } => { serviceId }
    ...
    actions: {
        ...
        create: {
            ...
            async handler(ctx) {
                ...
                let statement = "MERGE (a:Account { uid: {serviceId}, ownerId: {ownerId} }) ";
        ...
        delete: {
            ...
            async handler(ctx) {
                ...
                let statement = "MATCH (a:Account { uid: {serviceId}, ownerId: {ownerId} }) ";
                statement += "WITH a ";
         ...
         * @param {String} serviceId
         * 
         * @returns {Object} account - { serviceId, label, token[] }
         */
        get: {
            acl: "before",
            params: {
            ...
            async handler(ctx) {
                ...
                statement += "RETURN a.uid AS serviceId, a.label AS label, token ";
                statement += ";";
         ...
         * @actions
         * 
         * @returns {Array} accounts[] - [{ serviceId, label, token[] }]
         */
        getAll: {
            acl: "before",
            async handler(ctx) {
        ...
        getAuthToken: {
            ...
            async handler(ctx) {
                ...
                // get token
                let statement = "MATCH (t:Token { uid: {tokenId}, ownerId: {ownerId} })-[:ASSIGNED]->(a:Account { uid: {serviceId}, ownerId: {ownerId} }) ";
        ...
        removeAuthToken: {
            ...
            async handler(ctx) {
                ...
                // get token
                let statement = "MATCH (t:Token { uid: {tokenId}, ownerId: {ownerId} })-[:ASSIGNED]->(:Account { uid: {serviceId}, ownerId: {ownerId} }) ";
        ...
        login: {
            ...
            async handler(ctx) {
                ...
                let statement = "MATCH (t:Token)-[:ASSIGNED]->(a:Account { uid: {serviceId} }) ";
                    ...
                    for (let i = 0; i < result.length; i++) {
                        ...
                        if (check) {
                            ...
                            try {
                                ...
                                if (res && res.token) {
                                    return {
                                        sessionToken: this.signedJWT({ type: "session_token", id: ctx.params.serviceId }),
                                        accessToken: res.token
                                    };
                                }
                            } catch (err) {
                                this.logger.error("Failed to retrieve access token", { serviceId: ctx.params.serviceId });
                            }
            ...
            async handler(ctx) {
                ...
                .then(decoded => {
                    if (decoded.type == "session_token" && decoded.id) {
                        ...
                        let params = {
                            serviceId: decoded.id
                        };
                        let statement = "MATCH (a:Account { uid: {serviceId} }) ";
                        statement += "RETURN a.uid AS serviceId, a.label AS label, a.ownerId AS ownerId ";
                        statement += ";";
